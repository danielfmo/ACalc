    ...
    def resultado(self):
        ...
        if self._tipo1 == "RECTO":
            ...
            if self._tipo2 == "INTERIOR":
                self._inv_teta = self._inv_alfa - self._a / \
                    (2*self._R*cos(self._alfa)) + (self._i/(2*self._R))
            ...
            # - a no caso de exterior
            self._L = self._R*cos(self._alfa)/cos(self._teta_r)
        ...
        if self._tipo1 == "HELICOIDAL":
            ...
            if self._tipo2 == "INTERIOR":
                self._inv_teta = self._inv_alfa - self._a / \
                    (2*self._R*cos(self._alfa)*cos(self._beta)) + \
                    (self._i/(2*self._R))
            elif self._tipo2 == "EXTERIOR":
                self._inv_teta = self._inv_alfa + self._a / \
                    (2*self._R*cos(self._alfa)*cos(self._beta)) - \
...
class EkEngrenagem (object):
    ...
    def resultado(self):
        ...
        if self._EE == self._dp:
            ...
            self._carreto['EK'] = self.calc_ek(
                self._z1, self._dentes_ek1)-self._carreto['erro']['folga']
        ...
        # Se FORA do entre-eixo de funcionamento
            ...
            self._carreto['EK'] = self._carreto['E'] - \
                self._carreto['erro']['folga'] + \
                (2*self._DELTA1*self._m*sin(self._alfa))
            self._carreto['erro']['fc'] = self._DELTA1
            self._carreto['de'] = self._m * \
                (self._z1/cos(self._beta)+2+(2*self._DELTA1)-(2*self._K2))
    ...
    def resultado(self):
        ...
        if self._EE == self._eef:
            ...
            if self._fixek1 == None:
                self._carreto['EK'] = self._carreto['EK'] - \
                    self._carreto['erro']['folga']
        ...
        # Se FORA do entre-eixo de funcionamento
            ...
            self._zv1 = self._z1 / \
                (cos(self._beta)*cos(self._beta)*cos(self._beta))
            self._zv2 = self._z2 / \
                (cos(self._beta)*cos(self._beta)*cos(self._beta))
            ...
            # Calculo normal
            if self._fixek1 == None and self._fixek2 == None:
                self._DELTA1 = 0.5*(self._zv2-self._zv1)/(self._zv2 +
                                                          self._zv1)+(self._W*self._zv1)/(self._z2+self._zv1)
                self._DELTA2 = self._W-self._DELTA1

                self._carreto['EK'] = self._carreto['E'] - \
                    self._carreto['erro']['folga'] + \
                    (2*self._DELTA1*self._m*sin(self._alfa))
                self._roda['EK'] = self._roda['E']-self._roda['erro']['folga'] + \
                    (2*self._DELTA2*self._m*sin(self._alfa))

            # calcular EK1 com EK2 fixo
            elif self._fixek1 == None and self._fixek2 == "ON":
                self._DELTA2 = (
                    self._ek2-self._roda['E']+self._roda['erro']['folga'])/(2*self._m*sin(self._alfa))
            ...
            # calcular EK2 com EK1 fixo
                ...
                self._DELTA1 = (
                    self._ek1-self._carreto['E']+self._carreto['erro']['folga'])/(2*self._m*sin(self._alfa))
                self._DELTA2 = self._W - self._DELTA1
                self._carreto['EK'] = self._ek1
                self._roda['EK'] = self._roda['E']-self._roda['erro']['folga'] + \
                    (2*self._DELTA2*self._m*sin(self._alfa))
            ...
            self._carreto['de'] = self._m * \
                (self._z1/cos(self._beta)+2+(2*self._DELTA1)-(2*self._K2))
            self._roda['de'] = self._m * \
                (self._z2/cos(self._beta)+2+(2*self._DELTA2)-(2*self._K2))
...
class EkSemfim (object):
    ...
    def calc_M(self):
        ...
        self._M2 = self._D1 - ((self._PN/self._z1/2) /
                               tan(self._alfa)) + self._a*(1/sin(self._alfa)-1)
    ...
    def calc_G(self):
        self._AX1 = -1*self._EE + \
            (self._a/(cos(self._alfaap)*cos(self._betaap)))
    ...
    def resultado(self):
        ...
        if self._fixM == None and self._fixG == None:
            ...
            while self._Gt < self._G:
                ...
                self._D2 = (2*self._a-self._m*pi*cos(self._betaap)*cos(self._alfaap)) / \
                    (2*cos(self._betaap)*cos(self._alfaap)
                     * (self._inv_teta-self._inv_alfa))
            ...
            if self._z1 % 2 != 0:
                self._M2 = sqrt(pow((self._M+self._a), 2) +
                                pow(self._EE, 2))-self._a
                self._D1 = self._M2 + \
                    (((self._PN/self._z1)/2)/tan(self._alfa)) - \
                    self._a*(1/sin(self._alfa)-1)
            else:
                self._D1 = self._M + \
                    (((self._PN/self._z1)/2)/tan(self._alfa)) - \
                    self._a*(1/sin(self._alfa)-1)
