...
class CalcSpiro (object):
    ...
    def passo1(self, kwargs):
        ...
        self._zp = sqrt(	pow(self._z1, 2) + pow((self._z2+self._z1 *
                                                 cos(radians(self._e)))/sin(radians(self._e)), 2)	)

        self._S1 = atan(	(self._z1/self._zp) /
                         sqrt(-pow(self._z1/self._zp, 2)+1)	)
        self._S2 = atan(	(self._z2/self._zp) /
                         sqrt(-pow(self._z2/self._zp, 2)+1)	)
